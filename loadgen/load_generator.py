...
class LoadGenerator(object):
    ...
    def measure_replicas(self):
        ...
        if platform.system() == 'Windows':
            os.system('taskkill /F /FI "WINDOWTITLE eq *measure_replicas*" 2>nul')
        else:
            os.system('pkill -f "measure_replicas"')
        # Monitor number of deployments.
        if platform.system() == 'Windows':
            # Windows: use subprocess to run in background
            subprocess.Popen(['python', 'evaluation/eval_utils/measure_replicas.py'], 
    ...
    def get_replicas(self):
        import platform
        # Read replica data.
        replica_file = 'service_replicas_count.json'
        if not os.path.exists(replica_file):
            return []
        with open(replica_file, 'r') as f:
            lines = f.readlines()
            replicas = [json.loads(x) for x in lines if x.strip()]
...
def kill_load_gen():
    import platform
    if platform.system() == 'Windows':
        os.system('taskkill /F /IM locust.exe 2>nul')
        os.system('taskkill /F /FI "WINDOWTITLE eq locust*" 2>nul')
    else:
...
def increase_open_files():
    ...
    import platform
    if platform.system() != 'Windows':
        # ulimit is Unix/Linux only
        os.system('ulimit -Sn 65535')
    # Windows doesn't need this - file handles are managed differently
