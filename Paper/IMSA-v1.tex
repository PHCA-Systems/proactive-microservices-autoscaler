...
The SLA threshold is derived empirically using a percentile-based methodology grounded in the operational reality that no contractual SLA exists for the Sock Shop benchmark. The system is run under comfortable load (constant pattern, 50 users) to observe the p95 latency the front-end naturally produces under normal operating conditions. The P75 of these observations is taken as the SLA threshold---this choice captures the upper bound of normal variance without being so tight that routine fluctuations trigger violations, balancing sensitivity (catching genuine violations) with specificity (avoiding false alarms from routine variance). For the GKE deployment, this methodology yields a threshold of 35.68ms. Front-end p95 latency is chosen as the signal because front-end is the user-facing entry point that every request passes through, aggregating the effect of all downstream service degradation into a single observable metric.
...
All experiments were conducted in evaluation mode, where scaling decisions are logged for analysis rather than executed via the Kubernetes API, enabling controlled comparison against the HPA baseline.
