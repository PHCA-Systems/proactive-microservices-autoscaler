...
Runs ONE proactive step test for 5 minutes to prove controller changes work.
"""
...
SETTLE_SECONDS = 30         # Shorter settle
...
def enable_proactive():
    print("  Enabling PROACTIVE controller...")
    autoscaling_v2 = client.AutoscalingV2Api()
    for svc in MONITORED_SERVICES:
        try: autoscaling_v2.delete_namespaced_horizontal_pod_autoscaler(name=f"{svc}-hpa", namespace=NAMESPACE)
        except Exception: pass
    subprocess.run(["kubectl", "--context=gke_grad-phca_us-central1-a_pipeline-cluster", "scale", "deployment", "scaling-controller", "-n", "kafka", "--replicas=1"], capture_output=True)
    time.sleep(10)
...
def main():
    print("\n=== QUICK PROACTIVE VALIDATION TEST ===")
    enable_proactive()
    reset_cluster()
