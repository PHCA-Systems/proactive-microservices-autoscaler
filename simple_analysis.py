#!/usr/bin/env python3
"""
Simplified Analysis Pipeline (without scipy)
Processes all 34 experiment runs and generates comparison tables.
"""
import json
import os
...
for f in sorted(RESULTS_DIR.glob("*.jsonl")):
    ...
    # Only process files from this run (run01-run05)
    if not any(f"run0{i}" in name for i in range(1, 6)):
        continue
    parts = name.split("_")
    if len(parts) < 3:
        continue
...
print("\n" + "=" * 120)
print("  FULL EXPERIMENT RESULTS - 34-Run Analysis")
print("=" * 120)
...
for pattern in PATTERNS:
    ...
    print(f"{'=' * 120}")
    print(f"  {'Service':<12} | {'Pro VR%':>9} | {'Rea VR%':>9} | {'Pro p95':>9} | {'Rea p95':>9} | {'Pro RepSec':>10} | {'Rea RepSec':>10} | {'Winner'}")
    print("  " + "-" * 98)
    ...
    for svc in MONITORED:
        ...
        print(f"  {svc:<12} | {avg_pro_vr:>8.1f}% | {avg_rea_vr:>8.1f}% | {avg_pro_p95:>7.1f}ms | {avg_rea_p95:>7.1f}ms | {avg_pro_rep:>10.0f} | {avg_rea_rep:>10.0f} | {winner}")
...
print(f"\n{'=' * 120}")
print("  Analysis complete (Note: Statistical tests require scipy)")
print(f"{'=' * 120}\n")
